...
<section class="thank" style="direction: rtl; font-family: 'Changa', sans-serif;">
	<div class="container">
		<div class="row">

			<h1 class="text-center" style="font-family: 'Changa', sans-serif;">Thank You</h1><hr>
			...
			if (isset($_GET['course'])) {
				...
				$phone_start = substr($phone01, 0 ,3);

				$starts = array("011","010","012","015");

				// if (in_array($phone_start, $starts)) {
				// 	$phone 	 = $_POST['phone'];
				// } else {
				// 	$phone = "15";
				...
				if (!ctype_alpha($name)) {
					$Errors[] = "Name Must be just letters";
				}
				...
				if (empty($phone01)) {
					$Errors[] =  "phone Must be filled";
				}

				 if (strlen($phone01) == 11 || strlen($phone01) == 14 ) {
				 	if (strlen($phone01) == 11 ) {
				 		if (in_array($phone_start, $starts)) {
							$phone 	 = $phone01;
						} else {
							$phone = $phone01;
				 	...
				 	$Errors[] =  "phone Must be 14 digit OR 11 digit";
				 }
				...
				if (empty($Errors)) {
					...
					$stmt = $con -> prepare("INSERT INTO participants (name , email , phone , course,R_time) VALUES (:name,:email,:phone,:course,now() )");
					$stmt -> execute(array(
						"name" 		=> $name ,
						"email"		=> $email,
						"phone"		=> $phone,
					...
					if ($count > 0) {
						echo "<div class='alert alert-success'>Thank You For Enroll with us</div>";
						echo "<a href='index.php' class='btn btn-info'>Go Home</a>";
					} else {
						echo "<div class='alert alert-danger'>Please Try Again Later</div>";
						echo "<a href='".$_SERVER['HTTP_REFERER']."' class='btn btn-info'>Try Again</a>";
					}

				} else {
					echo "<a href='".$_SERVER['HTTP_REFERER']."' class='btn btn-warning'>Go Back</a>";
				}
